...
/**
 * Gate list + admit sheet — hosted at the top of Grow.
...
export function GrowGatePanel({ ownerFp }: Props) {
  ...
  if (focus) {
    return (
      <div data-testid="grow-gate-admit" style={{ marginBottom: 20, paddingBottom: 16, borderBottom: `1px solid ${E.border}` }}>
        <p style={{ margin: 0, fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: E.accent }}>
          {GATE_COPY.title}
        </p>
        <h3 style={{ margin: '8px 0 0', fontSize: 18, fontWeight: 400, color: E.text }}>
          Admit as Known
        </h3>
        ...
        <code style={{ display: 'block', marginTop: 4, fontSize: 12, color: E.accent, wordBreak: 'break-all', fontFamily: E.fontMono }}>
        ...
        {(
          ...
          <label key={v} style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 6, fontSize: 13, color: E.text }}>
            <input
              type="radio"
              name="gate-verify"
              checked={verify === v}
              onChange={() => setVerify(v)}
            />
            {label}
          </label>
        ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 16 }}>
          <button type="button" data-testid="grow-gate-admit-confirm" disabled={busy} onClick={() => void onAdmit()} style={btnStyle(true)}>
  ...
  return (
    <div data-testid="grow-gate-list" style={{ marginBottom: 20, paddingBottom: 16, borderBottom: `1px solid ${E.border}` }}>
      <p style={{ margin: 0, fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: E.accent }}>
        {GATE_COPY.title}
