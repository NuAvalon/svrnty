...
- **vCard export** is a **phone book**: name, emails, phones, urls, org/title/address, notes, handles. **Stripped:** trust, fingerprint, public key, owner-local group tags, they_trust, share settings. JSON export is the full local dump (auth-gated) — not the phone-book path.
